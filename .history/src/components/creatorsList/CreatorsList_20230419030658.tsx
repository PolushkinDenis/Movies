  ...
  return (
    <div className="creatorsList _container">
      ...
      <ul className="creatorsList__list">
        {creatorsTest.map((creator, index) => (
          <li key={index} className="creatorsList__list-item">
            <img src={creator.imageUrl} alt="name" className="creatorsList__list-item-image" />
            <p className="creatorsList__list-item-name">
              {creator.name.split(' ')[0]}
              <br />
              {creator.name.split(' ')[1]}
            </p>
            <div className="creatorsList__list-item-role">{creator.role}</div>
          </li>
        ))}
        <li className="creatorsList__list-more">Еще</li>
      </ul>
    </div>
  );
};
